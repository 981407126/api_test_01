...
#         param params : 查询参数 如page=1&limit=20&sort=add_time&order=desc
#         例如:http://121.196.13.152:8080/admin/auth/login
#         """
#         if params:
#             full_url = BASE_URL + path + params
#             return full_url
...
#             return result
#         except Exception as e:
#             logger.error("请求get方法异常,返回的数据为{}".format(result))
#
#     # 重写post方法
#     def post(self,url,data,):
#         """
...
#         """
#         处理请求头
#         :return:返回的是一个字典形式的请求头，多包括了Content-Type,X-Litemall-Admin-Token
#         """
#         token = cache.get('token') # 从缓存中获取token值
#         headers = {'Content-Type':'application/json'}
#         if token:
...
# if __name__ == '__main__':
#     base = Base()
#     # print(base.get_url(path='/admin/admin/update'))
#     # print(base.get_url(path='/admin/admin/list',params='?page=1&limit=20&sort=add_time&order=desc'))
#     # ---------------------------------------------------------------------------------------------------
#     # login_url = base.get_url('/admin/auth/login')
#     # login_data = {"username":"admin123","password":"admin123"}
#     # print(base.post(login_url,login_data))
